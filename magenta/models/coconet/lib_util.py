...
def sample(p, axis=None, temperature=1, onehot=False):
  ...
  u2 = np.random.random([p.shape[d] if d != axis else 1 for d in range(p.ndim)])
  ...
  i2 = np.argmax(u2 * totalmasses < cmf, axis=axis)

  onehot1 = to_onehot(i, axis=axis, depth=p.shape[axis])
  onehot2 = to_onehot(i2, axis=axis, depth=p.shape[axis])

  onehottot = onehot1

  (batch_idx, time_idx, pitch_idx, instr_idx) = onehot1.shape
  for ii in range(batch_idx):
    for jj in range(time_idx):
      for kk in range(instr_idx):
        if all(onehot1[ii, jj, mm, kk]==onehot2[ii, jj, mm, kk] for mm in range(pitch_idx)):
          continue
        else:
          onehottot[ii, jj, :, kk] += onehot2[ii, jj, :, kk]
