...
class PianorollEncoderDecoder(object):
  ...
  def encode_list_of_lists(self, sequence):
    ...
    custom_channels = len(sequence[0])
    ...
    for raw_t, chord in enumerate(sequence):
      ...
      for i in range(custom_channels):
        ...
        if isinstance(pitch, float) or isinstance(pitch, np.float16):
          amount_of_notes_for_this_instrument = 1
        else:
          amount_of_notes_for_this_instrument = len(pitch)
        temp = pitch
        for note in range(amount_of_notes_for_this_instrument):
          if amount_of_notes_for_this_instrument > 1:
            pitch = temp[note]
