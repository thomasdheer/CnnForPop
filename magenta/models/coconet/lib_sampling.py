...
class BaseSampler(lib_util.Factory):
  ...
  def __call__(self, pianorolls, masks, priming=False, timeExtension=False):
    """Sample from model.
    ...
    with lib_util.timing(label):
      return self.run_nonverbose(pianorolls, masks, priming, timeExtension)

  def run_nonverbose(self, pianorolls, masks, priming, timeExtension):
    label = "%s_sampler" % self.key
    with self.logger.section(label):
      return self._run(pianorolls, masks, priming, timeExtension)
...
class IndependentSampler(BaseSampler):
  ...
  def _run(self, pianorolls, masks, priming, timeExtension):
    bat, leng, pitc, instr = pianorolls.shape

    predictions = self.predictor(pianorolls, masks)
    samples = self.sample_predictions(predictions)
    pianorolls_sample = np.where(masks, samples, pianorolls)

    firstbartimestep = int(leng/4)
    pr_previous_step_firstbar = pianorolls[:, 0:firstbartimestep,:, :]
    pr_new_samples_lastbars = pianorolls_sample[:, firstbartimestep:leng, :, :]
    if priming == True:
      pianorolls = np.concatenate((pr_previous_step_firstbar, pr_new_samples_lastbars), axis = 1)
    else:
      pianorolls = pianorolls_sample
    self.logger.log(pianorolls=pianorolls, masks=masks, predictions=predictions)
    return pianorolls
...
class GibbsSampler(BaseSampler):
  ...
  def __init__(self, **kwargs):
    ...
    self.priming = kwargs.pop("priming")
    self.timeExtension = kwargs.pop("timeExtension")
  ...
  def _run(self, pianorolls, masks, priming, timeExtension):
    ...
    if priming == True:
      firstbartimestep = int(leng / 4)
      pr_previous_step_firstbar = pianorolls[:, 0:firstbartimestep, :, :]
      pr_first_bar_and_zeros = np.concatenate((pr_previous_step_firstbar, np.zeros((bat, leng-firstbartimestep, pitc, instr))),axis=1)
      pianorolls = pr_first_bar_and_zeros

    if timeExtension == False:
      with self.logger.section("sequence", subsample_factor=10):
        for s in range(int(num_steps)):
          ...
          pianorolls = self.sampler.run_nonverbose(pianorolls, inner_masks, priming, timeExtension)
          self.logger.log(
              pianorolls=pianorolls, masks=inner_masks, predictions=pianorolls)
    else:
      sampling_blocks = 5
      bb, tt, pp, ii = pianorolls.shape
      quarter_index = int(tt/4)
      print('quarter index is:', str(quarter_index))
      blank_pr = np.zeros((bb, quarter_index, pp, ii))
      blank_mask = np.ones((bb, quarter_index, pp, ii))
      print('shape of quarter blank pianoroll is:', str(blank_pr.shape))
      extended_pr = np.zeros((bb, 2*tt, pp, ii))
      for i in range(sampling_blocks):
        if i==0:
          with self.logger.section("sequence", subsample_factor=10):
            for s in range(int(num_steps)):
              print(".", end="")
          ...
          extended_pr[:,0:tt,:,:] = pianorolls
        else:
          block_pr = np.concatenate((pianorolls[:,quarter_index:tt,:,:],blank_pr),axis=1)
          with self.logger.section("sequence", subsample_factor=10):
            for s in range(int(num_steps)):
              print(".", end="")
              pm = self.schedule(s, num_steps)
              inner_masks = self.masker(
                ...
                outer_masks=masks,
                separate_instruments=self.separate_instruments)
              pianorolls = self.sampler.run_nonverbose(block_pr, inner_masks)
              block_pr = pianorolls
              self.logger.log(
                pianorolls=pianorolls, masks=inner_masks, predictions=pianorolls)
          extended_pr[:, i*quarter_index:tt+i*quarter_index,:,:] = pianorolls
          print('written to timeindexes:',str(i*quarter_index),'to',str(tt+i*quarter_index))
      pianorolls = extended_pr

    print('pianorolls shape after sampling:', str(pianorolls.shape))
    return pianorolls
