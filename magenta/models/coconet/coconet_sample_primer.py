...
flags.DEFINE_string("prime_midi_fpath", None,
                    "Path to midi file to be extended.")
...
flags.DEFINE_bool("priming", False, "Prime the sample on a given MIDI sequence.")
flags.DEFINE_bool("timeExtension", False, "Extend the sample via moving-window sampling")
...
def main(unused_argv):
  ...
  midi_outs = generator.run_generation(
      gen_batch_size=FLAGS.gen_batch_size, piece_length=FLAGS.piece_length, midi_in=FLAGS.prime_midi_fpath, priming=FLAGS.priming, timeExtension=FLAGS.timeExtension)
  ...
  with tf.gfile.Open(intermediate_steps_path, "rb") as p:
    foo = np.load(p)                # foo: metasyntactic variable, just a placeholder variable
...
class Generator(object):
  ...
  def run_generation(self,
    ...
    if midi_in is not None: # and "midi" in self.strategy_name.lower():
      midi_in = pretty_midi.PrettyMIDI(midi_in)
      pianorolls = self.strategy(shape, midi_in, priming, timeExtension)
...
class BaseStrategy(lib_util.Factory):
  ...
  def __call__(self, shape, midi_in, priming, timeExtension):
    label = "%s_strategy" % self.key
    with lib_util.timing(label): # define context with 'with' keyword
      with self.logger.section(label):
        return self.run(shape, midi_in, priming, timeExtension)
...
class IgibbsStrategy(BaseStrategy):
  ...
  def run(self, shape, midi_in, priming, timeExtension):
    pianorolls = self.decoder.encode_midi_to_pianoroll(midi_in, shape)
    dummy, masks = self.blank_slate(shape)
    sampler = self.make_sampler(
        "gibbs",
        priming=priming,
        ...
        sampler=self.make_sampler("independent", temperature=FLAGS.temperature),
        schedule=lib_sampling.YaoSchedule())
    pianorolls = sampler(pianorolls, masks, priming, timeExtension)
    return pianorolls
