...
class CoconetGraph(object):
  ...
  def build(self):
    """Builds the graph."""
    featuremaps = self.get_convnet_input()              # Returns concatenates masked out pianorolls with their masks
    print('FEATURE MAP SHAPE: ' + str(featuremaps.shape))
    self.residual_init()
  ...
  def apply_batchnorm(self, x):
    ...
    if self.is_training:
      ...
      # tf.nn.moments calculates the mean and variance of x, along certain axes. 0: along column, 1: along row. keepdims: produce moments with the same dimensionality as the input.
      mean, variance = batchmean, batchvariance
      updates = [
          popmean.assign_sub(decay * (popmean - mean)),        # update ref by subtracting value from it
