...
class Hyperparameters(object):
  ...
  _defaults = dict(
      ...
      batch_norm=True,
      batch_norm_variance_epsilon=1e-7,   # 1e-7
      # Initialization.
      init_scale=0.1,  # 0.1
      # Model architecture.
      ...
      # num_dilation_blocks.
      num_layers=28,                # default: 28
      num_filters=256,              # default: 256
      use_residual=True,
      ...
      optimize_mask_only=False,   # False
      #use_softmax_loss=True,     # was commented at first
      rescale_loss=True,    # True
      # Training.
      # learning_rate=2**-6,
      learning_rate=2**-4,  # for sigmoids. default: 2**-4 (previously: 0.3)
...
class Straight(Architecture):
  ...
  def __init__(self, input_depth, num_layers, num_filters, num_pitches,  # pylint:disable=unused-argument
    ...
    def _add(**kwargs):
      self.layers.append(kwargs)    # kwargs: {'filters': [3, 3, 8, 128]}

    _add(filters=[3, 3, input_depth, num_filters])              # 3, 3
    for _ in range(num_layers - 3):
      _add(filters=[3, 3, num_filters, num_filters])            # 3, 3
    _add(filters=[2, 2, num_filters, num_filters])              # 2, 2
    _add(
        filters=[2, 2, num_filters, output_depth], activation=lib_util.identity)        # 2, 2
